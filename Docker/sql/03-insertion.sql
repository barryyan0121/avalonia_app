...
SELECT name,
       FLOOR(RAND() * 10) + 10             AS target_amount, -- Random target amount between 10 and 20
...
FROM
    ...
    (SELECT 1 AS repeat_offset
     ...
     SELECT 9
     UNION ALL
     SELECT 10) AS repeats -- This set will be further filtered
WHERE
    RAND() < 0.5; -- Randomly filter out about half of the entries for more varied counts